...
parser.add_argument('--pretrained_model_name_or_path', type=str, default="output_models/GRM-Llama3.2-3B-rewardmodel-ft_mixture_BT_stanford_shp_pairwise_heads10_lr2e-3_loss_loadBalance0.5_seed0")
parser.add_argument('--num_heads', type=int, default=10)
parser.add_argument('--data', type=str, default='anthropic_rlhf_hh_pairwise')
...
elif 'rpr_per_category_pairwise_add_criterion' in args.data:
    comps, rm_types = eval_utils.process_rpr_per_category_pairwise_add_criterion()
elif 'reward_bench_pairwise_augmented_context' in args.data:
    comps, rm_types = eval_utils.process_reward_bench_pairwise_add_criterion()
elif 'reward_bench_pairwise' in args.data:
    comps, rm_types = eval_utils.process_reward_bench_pairwise()
    ...
    for rm_id, rm_type in enumerate(rm_types):
        ...
        for i in range(num_heads):
            ...
            for item in tqdm(ds):
                ...
                router_weight = F.one_hot(torch.tensor(i), num_classes=num_heads).float().cuda()
            ...
            print(f'Head {i} Accuracy: {correct}/{len(ds)}, {correct/len(ds)}')
            res[rm_type].append(correct/len(ds))

df = pd.DataFrame(res)
df.to_csv(f'single_head-{args.data}.csv')
print('='*30)
print(df)
print('\n\n')
